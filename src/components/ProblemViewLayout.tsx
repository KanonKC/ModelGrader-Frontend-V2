...
const ProblemViewLayout = ({
	...
	useEffect(() => {
		const allowedLanguages = problem?.allowed_languages;
		if (problem && allowedLanguages && allowedLanguages.length > 0) {
			...
			if (previousSelectedLanguage) {
				...
				const autoSelectedLanguage = ProgrammingLanguageOptions.filter(
					(lang) => allowedLanguages.includes(lang.value)
				)[0].value;
			...
			<ResizablePanel defaultSize={50} className="w-1/2 mr-5">
				<div className="flex justify-between mb-1 items-center">
					<div className="flex gap-2">
						<Combobox
							...
							options={ProgrammingLanguageOptions.filter(
								(lang) =>
									problem?.allowed_languages &&
									problem?.allowed_languages.includes(
										lang.value
									)
							)}
							onSelect={(value) => {
				...
				<div className="flex justify-between mt-1 gap-2">
					<PreviousSubmissionsCombobox
						bestSubmission={previousSubmissions?.best_submission}
						submissions={
							previousSubmissions?.submissions as SubmissionModel[]
						}
					...
					<Button
						disabled={
							grading ||
							!submitCodeValue ||
							ProgrammingLanguageOptions.filter(
								(lang) =>
									problem?.allowed_languages &&
									problem?.allowed_languages.includes(
										lang.value
